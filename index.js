...
class Query {
    ...
    static async getTokenBalance(api, address, tokenId) {
        ...
        return {
            free: util.isHex(free.toString())
                ? util.hexToBn(free.toString())
                : new util.BN(free.toString()),
            reserved: util.isHex(reserved.toString())
                ? util.hexToBn(reserved.toString())
                : new util.BN(reserved.toString()),
            frozen: util.isHex(frozen.toString())
                ? util.hexToBn(frozen.toString())
                : new util.BN(frozen.toString())
        };
    }
        ...
        try {
            const unsub = await tx.signAndSend(account, {
                ...
                if (result.status.isFinalized) {
                    ...
                    const unsubscribeNewHeads = await api.rpc.chain.subscribeFinalizedHeads(async (lastHeader) => {
                      ...
                      $([${truncatedString(inclusionBlockHash)}])`);
                            ...
                            console.info(`Retry [${retries}] Tx: [${truncatedString(tx.hash.toString())}] current: #${lastHeader.number} [${truncatedString(lastHeader.hash.toString())}] finalized in: #${inclusionBlockNr} [${truncatedString(inclusionBlockHash)}] `);
                        }
                        else {
...
class Mangata {
    ...
    async connectToNode(urls) {
        const provider = new ws.WsProvider(urls, 5000);
        const api$1 = await api.ApiPromise.create(types.options({ provider, throwOnConnect: true, throwOnUnknown: true }));
        return api$1;
    }
    /**
